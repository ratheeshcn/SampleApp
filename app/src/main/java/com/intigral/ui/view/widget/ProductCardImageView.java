...
/**
 * Created by retheeshcn on 6/29/17.
 * A custom class for create an image view for the product card. only top side of this imageview
 * will be rounded
 */
...
public class ProductCardImageView extends AppCompatImageView {
    ...
    public ProductCardImageView(Context context) {
        super(context);
    }

    public ProductCardImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ProductCardImageView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.ProductCardImageView, defStyle, 0);

        final int index = a.getInt(R.styleable.ProductCardImageView_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        mLeftTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.ProductCardImageView_sriv_left_top_corner_radius, 0);
        mRightTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.ProductCardImageView_sriv_right_top_corner_radius, 0);
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.ProductCardImageView_sriv_left_bottom_corner_radius, 0);
        mRightBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.ProductCardImageView_sriv_right_bottom_corner_radius, 0);
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.ProductCardImageView_sriv_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a
                .getColorStateList(R.styleable.ProductCardImageView_sriv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(R.styleable.ProductCardImageView_sriv_oval, false);
        a.recycle();
